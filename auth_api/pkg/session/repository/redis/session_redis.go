...
func (sm *SessionRedisManager) Create(JWTAuthToken string, JWTRefreshToken string, userID int) error {
	...
	sm.mu.Lock()
	result, err := redis.String(sm.redisConn.Do("SET", mkey, dataSerialized, "EX", 1*24*60*60))
	sm.mu.Unlock()
